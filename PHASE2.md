...
Критический путь: **2.1 → 2.2 → 2.3 → 2.4**. После 2.4 — 2.5 и 2.6 можно параллелить.

Почему именно такой порядок:
- Folders идут первыми, чтобы Search сразу индексировал с `folderId` и не пришлось переделывать индекс.
- Тесты идут сразу после Folders (2.2) — код свежий, testable seams очевидны. Search Worker (2.4) потом расширит тест-сюит новым портом.
...
## Phase 2.3 — Юнит-тесты: domain + application

**Что нужно получить.** Vitest настроен и работает. Unit-тесты для `domain/folder/`, `domain/note/`, `application/folder/`, `application/note/`. Понимание того, что Clean Architecture даёт testable seams «бесплатно» — прямо на существующем коде.

**Охват в рамках проекта.** Тестируем только `domain/` и `application/` — осознанный выбор для учебного пет-проекта. Именно здесь живут бизнес-правила и инварианты. `infrastructure/` и `presentation/` unit-тестами не покрываем.

**Почему именно сейчас.** Phase 2.2 закончена: `domain/folder/` и `application/folder/` написаны и хорошо знакомы. Лучший момент для тестов — пока код свежий и мотивация каждого правила ещё понятна. Когда в Phase 2.4 появится `SearchEnginePort`, тест-сюит расширится на него — тогда будет наглядно видно, почему DI так удобен для тестирования.

**Инструменты для изучения.**
- **Vitest** — нативная поддержка Vite, ESM, path aliases из `vite.config.ts`. Чем отличается от Jest?
- **`vi.fn()`** — создание мок-функции. Что такое `mockReturnValue`, `mockResolvedValue`, `mockImplementation`?
- **`describe` / `it` / `expect`** — структура теста. Что такое AAA-паттерн (Arrange, Act, Assert)?

**Стратегия тестирования.**

| Слой | Тип теста | Как |
|------|-----------|-----|
| `domain/` | Unit | Чистые функции — нет моков вообще |
| `application/` | Unit | Зависимости (репозитории) — мок-объекты через `vi.fn()` |

**Вопросы для размышления.**
- Что стоит тестировать в `domain/folder/folder.rules.ts` без каких-либо моков? Начни с `canMoveFolder` (детект цикла) — почему это тест бизнес-правила, а не теста инфраструктуры?
- Как написать unit-тест для use case `createFolder`, если он зависит от `IFolderRepository`? Что должен делать мок-репозиторий — просто возвращать данные или ещё и проверять вызовы?
- Разница между `vi.fn()` и `vi.mock()`: когда достаточно первого, когда нужно второе?
- Что тестировать в `application/note/` — каких сценариев больше всего боишься пропустить?
- Какой coverage осмыслен для учебного проекта? Погоня за 100% или осознанное покрытие критичных правил?

**Подводные камни.**
- Vitest по умолчанию запускается в Node.js среде — `window`, `navigator`, `indexedDB` недоступны. Для `domain/` и `application/` это нормально — они чистые.
- Path aliases (`@domain/*`) нужно прописать в конфиге Vitest или унаследовать из `vite.config.ts` через `mergeConfig`. Изучи `defineConfig` + `resolve.alias` в `vitest.config.ts`.

**Acceptance.**
- `pnpm test` проходит, покрывает `canMoveFolder` (включая сценарий цикла), `createFolder` (happy path + ошибка дубликата), `deleteFolder` (cascade).
- Тесты запускаются без браузера в Node.js.
- Нет ни одного теста, который тестирует реализацию вместо поведения.

---

## Phase 2.4 — Search Worker + первый DI-порт
...
- Сериализация индекса на диск — сразу в Phase 2.4 или отложить? (Подсказка: это tech-debt, отложи в `TECH_DEBT.md`.)
...
**После 2.4 — расширить тесты.** Теперь появился `SearchEnginePort` и DI. Хороший момент дописать тест для `application/search/` с мок-реализацией порта — это покажет на практике, почему DI так удобен для тестирования.
...
## Phase 2.6 — Streams export / import
