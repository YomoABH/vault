...
src/
├── domain/
│   ├── note/           # Note entity, NoteContent value object, WikiLink, rules
│   ├── graph/          # NoteGraph entity, graph rules
│   ├── search/         # SearchIndex entity, tokenizer
│   ├── sync/           # ChangeSet, DevicePair, merge strategy
│   └── ports/          # Interfaces: NoteRepository, FileStorage, SearchEngine,
...
**Phase 2: Performance** — Web Workers (search, parsing), View Transitions, Streams (export/import).
...
Ports and DI are introduced gradually: a port interface is created when a second implementation appears or when a worker needs to reuse the same logic.
